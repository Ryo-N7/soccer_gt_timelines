...
Most of the plots you see inserted into {gt} or other R tables in general are spark lines so I though about including them in mine too. 
...
add emoji to further highlight (regular version uses a star emoji instead due to HTML-to-PNG conversion issues with the soccer emoji)

title text: Experimented with different sizes . Still not sure how the text alignment (especially vertical) works so I didn't want to try anything too drastic.
...
inexperience with HTML/CSS to get team logos to slot in on either side of the title text which was only solved after hours upon hours of attempts.

necessity to be careful about what colors to use as the main team colors (yellow for example is strictly off-limits). Potential for a dark theme design to alleviate some concerns with using yellow or other brighter colors that don't work well with white text.

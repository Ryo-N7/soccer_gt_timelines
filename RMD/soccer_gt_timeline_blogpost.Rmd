...
In this blog post I'll be talking about my submission to the 2020 RStudio Table contest.
...
For the title headline text I experimented with a few different sizes depending on the content. The actual score being the biggest in size, then the team names, and then the xG totals for either team in the smallest font at the far ends. I'm still not sure how the text alignment (especially vertical) works so I didn't want to try anything too drastic. I'm still not very fond of the fact that the parentheses around the xG values kind of dip below the other letters, especially the larger text. I am also still vacillating between adding a bit more color to either "xG" or the xG values themselves along their respective team colors. 

column text: column headers in slightly grey to differentiate the text from the title. but still not sure how it looks like as part of the whole table... 


 focus on **actual** score line then down to **xG**. I have Expected Points (xPoints) in the other graphic but for the stand-alone version of the tables it might be prudent to include them somehow in a later iteration. 
...
match_gt_xG_timeline <- 
  ...
  tab_header(   
    title = gt::html("<p><img src='https://i.imgur.com/RlXYW46.png' alt='LFC' style='float:left;height:190px;'><span style='font-size:35px'>(3.15 xG)</span> <b style='color:#d00027; font-size:55px'>Liverpool</b> <span style='font-size:70px'>4 - </span><span style='font-size:70px'>3</span> <b style='color:#1D428A; font-size:55px'>Leeds</b> <span style='font-size:35px'>(0.27 xG)</span><img src='https://i.imgur.com/r6Y9lT8.png' alt='LUFC' style='float:right;height:190px;'> 
</p><span style='font-size:40px'>Premier League 2020-2021: Matchday 1 (Sep 12 2020)</span>"))
```
...
We can wrap this all up in a neat function that takes a few useful and flexible arguments so that we can use the code and apply it to other matches. We'll use the inputs we created up to Part 1 as the arguments to pass to the function so the function will contain all the code from Part 2 and Part 3.
...
create_timeline_table <- function()
```
...
gtsave(match_gt_xG_timeline, here::here(glue("RMD/output/{hom}{awa}_match_gt_shotmap13.html")))
```
...
```{r eval=FALSE}
markdown::rpubsUpload(title = "gt-test4.html", htmlFile = here::here("RMD/output/LivLee_match_gt_shotmap13.html"))
```

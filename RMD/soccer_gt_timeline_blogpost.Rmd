...
In the github repo I've included data for a couple of matches so you can play around with it. Nothing is really stopping you from using StatsBomb, Opta, InStat, WyScout, etc. data to create these tables you just need a few extra steps to prepare the data to how I've done it. 


```{r}
raw_match_df <- understatr::get_match_shots(14090)
...
```{r}
match_shots_data_clean <- raw_match_df %>% 
...
minute <- c(0:last_min)
team_name <- c(match_shots_data_clean$h_team,
               match_shots_data_clean$a_team)

match_rollsumxG <- match_shots_data_clean %>% 
...
Taking the shots_df data frame above, expanding it out for every minute of the game and the applying the 

cumsum() and lag() functions. Be very careful and make sure that the rows are ordered properly before running this function.

```{r}
glimpse(roll_sum_df %>% select(minute, xG, rollsum))
```
...
match_df <- shots_df %>% 
  ...
  mutate(team_color = case_when(
    team_name == home_team ~ home_color,
    team_name == away_team ~ away_color,
    TRUE ~ NA_character_
  )) %>% 
  ## set IDs so that shots can always be arranged in the proper order
...
gtsave(match_gt_xG_timeline, here::here(glue("RMD/output/{hom}{awa}_match_gt_shotmap4.html")))
```

...
The variables defined below will be used throughout the code and some are appended to the data frames we'll be creating and cleaning. They'll also be used as arguments for a function we'll create that takes in all the code that'll be described throughout this blog post. 
...
You have to adjust xG for a single sequence of possession so I regularly use the code in [Joe Gallagher](https://twitter.com/joedgallagher)'s [blog post](https://jogall.github.io/2019-01-16-adjusted-xG/) for adjusting them whenever there's a sequence with a rebound. 
...
```{r eval=FALSE}
create_timeline_table(shots_df, rollsum_df, home_stats, away_stats,
                      home_team, away_team, home_color, away_color, 
                      match_date, league_year, matchday)
```
...
* [David Smale](https://twitter.com/committedtotape) for providing the inspiration and initial #TidyTuesday code example for inserting ggplots into {gt} via the `text_transform()` function. 

* [Rich Iannone](https://twitter.com/riannone) for {gt}

* [Joe Gallagher](https://twitter.com/joedgallagher) for the R code for adjusting xG per possession sequence.

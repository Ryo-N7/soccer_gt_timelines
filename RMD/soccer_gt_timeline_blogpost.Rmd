...
So now we have a data frame with a basic shot map for each team for each row of shots. However, all of the 


we can get those back by `unnest()`-ing the "data" column so all the other shot information is unpacked 
...
dfdfdf <- match_df_basic_plot %>% 
  ...
  mutate(complete_plot = pmap(list(x, y, xG, plot), add_xG_shot),
         ggplot = NA) %>% 
  select(-plot) %>% 
  left_join(match_rollsum_simple %>% select(id, rollsum_goal), by = "id")
```
...
As seen in our last `glimpse()` call we have a lot of columns that we won't really need for the table so we'll do some final cleaning up before we get to the {gt} table code. 

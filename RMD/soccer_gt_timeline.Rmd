...
createTimelineTableGT <- function(
  ...
  match_df <- shots_df %>% 
    mutate(x = case_when(
      h_a == "a" ~ X * 587,#587 - (X * 587),
      h_a == "h" ~ X * 587,
      TRUE ~ 0),
      y = case_when(
        h_a == "a" ~ Y * 373,#373 - (Y * 373),
        h_a == "h" ~ Y * 373,
        TRUE ~ 0)) %>%
  ...
  match_gt_xG_timeline <- match_shots_table_df %>% 
    gt() %>% 
    tab_header(   # <p><span style='font-size:40px'>Shots Timeline</span></p>
      title = gt::html(glue::glue("<p><img src='Liverpool_FC.png' alt='LFC' style='width:190px;vertical-align:middle;margin:50px 150px 50px 100px;'></span><img src='Leeds_United_FC.png' alt='LUFC' style='width:190px;vertical-align:middle;margin:50px 100px 50px 150px;'></p>")),
      #subtitle = gt::html(glue::glue("<span style='font-size:40px'>Premier League 2020-2021: Matchday 1 ({match_date})</span>"))
      subtitle = gt::html(glue::glue("<span style='font-size:35px'>({home_stats$xG} xG)</span> <b style='color:{home_color}; font-size:55px'>{home_team}</b> <span style='font-size:70px'>{home_stats$GOALS} - </span><span style='font-size:70px'>{away_stats$GOALS}</span> <b style='color:{away_color}; font-size:55px'>{away_team}</b> <span style='font-size:35px'>({away_stats$xG} xG)<p><span style='font-size:40px'>Premier League 2020-2021: Matchday 1 ({match_date})</span></p>"))
    ) %>% 
    ## title style
    tab_style(
      style = list(
    ...
    tab_style(
      ...
      locations = cells_body(
        columns = TRUE) # vars(team_name)
    ) %>% 
    ## add Goal result emoji
    text_transform(
      locations = cells_body(
        columns = vars(result),
        rows = result %in% c("GOAL", "OWN GOAL")),
      fn = function(x) paste(x, soccer_ball) # soccer_ball   soccer_ball_png
    ) %>% 
    ## add plots
    text_transform(
      locations = cells_body(columns = vars(ggplot)),
      fn = function(x) {
        map(match_shots_table_df$complete_plot, ggplot_image, height = px(150), aspect_ratio = 2) # 30 4
      }
    ) %>% 
    ## Rename cols
    cols_label(
      minute = "Minute", team_name = "Team", player_name = "Player", 
      result = "Result", xG = "xG", rollsum = "Cumulative xG", ggplot = "Shot Map", situation = "Situation", 
